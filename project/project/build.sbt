...
onLoad in Global := {
  ...
  val setUpAlternativeGlobal = { (state: State) =>
    ...
    if (globalBase.isDefined) state
      ...
      val contents0 = IO.read(sbtOptsFile)
      val newDef = s"${SbtGlobalPlugins}=${globalPlugins.getAbsolutePath}"
      val contents = s"$contents0${System.lineSeparator}$newDef"
      ...
      mirrorDirectory(currentPluginsBase, globalPlugins, logger)
...
def mirrorDirectory(dir: File, target: File, logger: Logger): Unit = {
